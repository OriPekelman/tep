...
module Tep
  class Events
    ...
    def run_start(host, backend_kind, model_name, model_path, config_json)
      ...
      line = "{" +
        Tep::Json.encode_pair_str("kind", "run_start") + "," +
        Tep::Json.encode_pair_str("schema", "toy/v1") + "," +
        Tep::Json.encode_pair_int("t", 0) + "," +
        Tep::Json.encode_pair_str("started_at", started) + "," +
        "\"host\":{" +
          Tep::Json.encode_pair_str("name", host) + "," +
          Tep::Json.encode_pair_str("os",   Sock.sphttp_os_kind) + "," +
          Tep::Json.encode_pair_str("arch", Sock.sphttp_arch_kind) +
        "}," +
        "\"backend\":{" + Tep::Json.encode_pair_str("kind", backend_kind) + "}," +
        "\"model\":{" +
          Tep::Json.encode_pair_str("name", model_name) + "," +
          Tep::Json.encode_pair_str("path", model_path) +
        "}," +
        "\"config\":" + config_json +
      "}"
      append_line(line)
    ...
    def inference(model, prompt_tokens, completion_tokens, wall_us, extra_json)
      ...
      extra = "{" +
        ...
        Tep::Json.encode_pair_int("prompt_tokens", prompt_tokens) + "," +
        Tep::Json.encode_pair_int("completion_tokens", completion_tokens) + "," +
        Tep::Json.encode_pair_int("latency_us", wall_us)
      caller_inner = ""
      ...
      extra = extra + "}"
      line = "{" +
        Tep::Json.encode_pair_str("kind", "eval") + "," +
        Tep::Json.encode_pair_str("phase", "serve") + "," +
        Tep::Json.encode_pair_int("t", rel_t) + "," +
        Tep::Json.encode_pair_str("name", "request") + "," +
        "\"extra\":" + extra +
      "}"
      append_line(line)
    end
    ...
    def run_end(reason)
      ...
      line = "{" +
        Tep::Json.encode_pair_str("kind", "run_end") + "," +
        Tep::Json.encode_pair_int("t", rel_t) + "," +
        Tep::Json.encode_pair_str("ended_at", ended) + "," +
        Tep::Json.encode_pair_str("reason", reason) + "," +
        "\"stats\":{" +
          Tep::Json.encode_pair_int("requests", @req_count) + "," +
          Tep::Json.encode_pair_int("errors", @err_count) + "," +
          Tep::Json.encode_pair_int("tokens_out", @tok_out) +
        "}" +
      "}"
      append_line(line)
    end
    ...
    def run_end_aggregated(reason)
      ...
      while i < lines.length
        ...
        if Tep.str_find(line_s, "\"kind\":\"eval\"", 0) >= 0 &&
          ...
          if extra_pos >= 0
            obj_end = Tep::Json.skip_container(line_s, extra_pos)
            extra_obj = line_s[extra_pos, obj_end - extra_pos]
            toks += Tep::Json.get_int(extra_obj, "completion_tokens")
          end
        end
        i += 1
      end
      ...
      out = "{" +
        ...
        Tep::Json.encode_pair_str("reason", reason) + "," +
        "\"stats\":{" +
          Tep::Json.encode_pair_int("requests", reqs) + "," +
          Tep::Json.encode_pair_int("errors", errs) + "," +
          Tep::Json.encode_pair_int("tokens_out", toks) +
        "}" +
      "}"
      append_line(out)
    end
