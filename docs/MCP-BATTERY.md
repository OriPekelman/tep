...
Three surfaces co-exist:

| URL | Format | Audience |
|---|---|---|
| `POST /mcp` | JSON-RPC 2.0 | MCP-native clients (Claude Code / OpenCode / Gravity) |
| `GET /llms.txt` | Plain markdown | LLM-readable catalog, also for humans skimming |
| `GET /openapi.json` | OpenAPI 3.0.3 | Non-MCP agents, Swagger UI, generic HTTP tooling |

All three are auto-generated by the translator from the same
`mcp_tool` / `mcp_resource` declarations; they stay in sync by
construction.

### AGENTS.md convention

Tep apps shipping MCP should ship an `AGENTS.md` file at the
repo root describing the agent-facing surface:

- What the app does (one paragraph).
- The discovery URLs (`/mcp`, `/llms.txt`, `/openapi.json`).
- The tool + resource catalog with capability requirements.
- Invariants the app maintains (status state machines, id
  monotonicity, append-only logs, ...).
- "Things you should NOT do" — guardrails an agent can't infer
  from the schema alone (concurrency caps, anti-patterns,
  semantic gotchas).
- Authorization model (how the agent gets the right caps).

This file is NOT auto-generated — it's hand-written, and Claude
Code reads it once per session before driving the app. See
`examples/experiments/AGENTS.md` for a worked example.
...
| **5.3** | `mcp_resource 'uri', "desc" do; on_read do; ...; end; end` -> `resources/list` + `resources/read` JSON-RPC + `GET /resources/<uri>` HTTP-direct. | Shipped (#67) |
| **5.4** | OpenAPI 3.0.3 auto-generation at `GET /openapi.json`. `AGENTS.md` convention documented. `examples/experiments` demo (mock training-runs manager with 4 tools + 2 resources + caps gating). | Shipping |
| **5.5+** | URI templating (`'experiment/{id}/metrics'`), streaming via `stream do |out| ... end`, MCP progress notifications. Likely chunked further. | Future |
