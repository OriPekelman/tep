...
class TestHttp < TepTest
  app_source <<~RB
    ...
    # Four workers: handlers make outbound HTTP back to the same
    # server, so worker A would block waiting for worker B to
    # service its inner request. SO_REUSEPORT distributes
    # connections by 4-tuple hash; with 4 workers and random
    # ephemeral source ports, the inner call lands on a free
    # worker with high probability.
    set :workers, 4
